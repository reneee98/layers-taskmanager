...
export function TaskReport({ taskId }: TaskReportProps) {
  ...
  const fetchFinanceData = async () => {
    if (!taskId || !taskId.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
      console.warn("Invalid taskId, skipping fetch:", taskId);
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (result.success && result.data) {
        console.log("Task finance data loaded:", result.data);
        setFinance(result.data);
      } else {
        console.error("Failed to fetch task finance data:", result.error || "Unknown error", result);
        setFinance(null);
      }
    } catch (error) {
      ...
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Skontrolujte, či má úloha uložené časové záznamy.
          </p>
        </div>
      </div>
    );
