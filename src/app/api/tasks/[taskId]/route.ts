...
export async function GET(
  ...
  try {
    ...
    if (!task) {
      console.log(
        `[GET /api/tasks/${taskId}] Task not found without workspaceId, trying to get workspace_id...`
      );

      // First, try to get workspace_id from the task using SECURITY DEFINER function
      // This bypasses RLS restrictions
      const { data: taskWorkspaceData, error: workspaceError } = await supabase.rpc(
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              "Nemáte oprávnenie na zobrazenie tejto úlohy. Skontrolujte, či máte 'tasks.read' permission.",
          },
          { status: 403 }
        );
      }
    ...
    if (task.project_id && workspaceId) {
      const hasProjectAccess = await canAccessProject(workspaceId, task.project_id, user.id);
      if (!hasProjectAccess) {
        return NextResponse.json(
          { success: false, error: "Nemáte prístup k tejto úlohe" },
          { status: 403 }
        );
      }
    }
...
export async function PATCH(
  ...
  try {
    ...
    if (
      validation.data.hourly_rate_cents !== undefined &&
      validation.data.hourly_rate_cents !== null
    ) {
      hourlyRateCents = validation.data.hourly_rate_cents;
    } else if (
      currentTask.hourly_rate_cents !== undefined &&
      currentTask.hourly_rate_cents !== null
    ) {
    ...
    if (
      validation.data.budget_cents !== undefined &&
      ...
      console.log(
        `Auto-calculated estimated_hours from budget: ${validation.data.estimated_hours}h (budget: ${budgetInEuros}€, rate: ${hourlyRate}€/h)`
      );
    }
    ...
    if (
      validation.data.estimated_hours !== undefined &&
      ...
      if (validation.data.budget_cents === undefined) {
        ...
        console.log(
          `Auto-calculated budget from estimated_hours: ${budgetInEuros}€ (hours: ${validation.data.estimated_hours}h, rate: ${hourlyRate}€/h)`
        );
      }
    }
    ...
    const budgetChanged =
      updateData.budget_cents !== undefined && updateData.budget_cents !== currentTask.budget_cents;
    ...
    const hourlyRateChanged =
      updateData.hourly_rate_cents !== undefined &&
    ...
    if (budgetChanged || estimatedHoursChanged || hourlyRateChanged) {
      console.log(
        "Budget, estimated_hours, or hourly_rate_cents changed, recalculating time entries..."
      );
      ...
      if (timeEntries && timeEntries.length > 0) {
        ...
        for (const entry of timeEntries) {
          ...
          await supabase.from("time_entries").update(updateData).eq("id", entry.id);
    ...
    const preservedFields = {
      due_date:
        task.due_date === currentTask.due_date
          ? "✅ preserved"
          : `❌ changed from ${currentTask.due_date} to ${task.due_date}`,
      start_date:
        task.start_date === currentTask.start_date
          ? "✅ preserved"
          : `❌ changed from ${currentTask.start_date} to ${task.start_date}`,
      end_date:
        task.end_date === currentTask.end_date
          ? "✅ preserved"
          : `❌ changed from ${currentTask.end_date} to ${task.end_date}`,
      status:
        task.status === currentTask.status
          ? "✅ preserved"
          : `❌ changed from ${currentTask.status} to ${task.status}`,
      priority:
        task.priority === currentTask.priority
          ? "✅ preserved"
          : `❌ changed from ${currentTask.priority} to ${task.priority}`,
      estimated_hours:
        task.estimated_hours === currentTask.estimated_hours
          ? "✅ preserved"
          : `❌ changed from ${currentTask.estimated_hours} to ${task.estimated_hours}`,
      actual_hours:
        task.actual_hours === currentTask.actual_hours
          ? "✅ preserved"
          : `❌ changed from ${currentTask.actual_hours} to ${task.actual_hours}`,
      assigned_to:
        task.assigned_to === currentTask.assigned_to
          ? "✅ preserved"
          : `❌ changed from ${currentTask.assigned_to} to ${task.assigned_to}`,
    };
    ...
    if (
      ...
      validation.data.description !== currentTask.description
    ) {
      await logActivity({
        workspaceId,
        userId: user.id,
    ...
    if (
      validation.data.project_id !== undefined &&
      validation.data.project_id !== currentTask.project_id
    ) {
      let oldProject = null;
      let newProject = null;
    ...
    if (
      validation.data.assigned_to !== undefined &&
      validation.data.assigned_to !== currentTask.assigned_to
    ) {
      const activityType = validation.data.assigned_to
        ? ActivityTypes.TASK_ASSIGNED
        : ActivityTypes.TASK_UNASSIGNED;
      const action = validation.data.assigned_to
        ? `Priradil úlohu používateľovi`
        : `Odstránil priradenie úlohy`;
    ...
    if (
      validation.data.due_date !== undefined &&
      validation.data.due_date !== currentTask.due_date
    ) {
      await logActivity({
        workspaceId,
        userId: user.id,
      ...
      if (
        ...
        if (isOverdue || isDueToday) {
          ...
          if (isOverdue) {
            ...
            console.log(
              `[Task ${taskId}] Auto-setting start_date and due_date to today (${todayStr}) because task is overdue`
            );
          } else {
            console.log(
              `[Task ${taskId}] Auto-setting start_date to today (${todayStr}) because task is due today`
            );
          }
          ...
          if (updateError) {
            ...
            if (reloadError) {
              ...
              console.log(
                `[Task ${taskId}] Successfully updated start_date to ${updatedTask.start_date}${isOverdue ? ` and due_date to ${updatedTask.due_date}` : ""}`
              );
              ...
              taskWithHourlyRate.project = updatedTask.project
                ? {
                    ...
                    hourly_rate: updatedTask.project.hourly_rate_cents
                      ...
                      : null,
                    fixed_fee: updatedTask.project.budget_cents
                      ? updatedTask.project.budget_cents / 100
                      : null,
                  }
                : null;
            }
          ...
          console.log(
            `[Task ${taskId}] Not auto-setting start_date: isOverdue=${isOverdue}, isDueToday=${isDueToday}`
          );
        }
      } else {
        console.log(
    ...
    if (
      validation.data.estimated_hours !== undefined &&
      validation.data.estimated_hours !== currentTask.estimated_hours
    ) {
      await logActivity({
        workspaceId,
        userId: user.id,
    ...
    if (
      validation.data.budget_amount !== undefined &&
      validation.data.budget_amount !== currentTask.budget_amount
    ) {
      await logActivity({
        workspaceId,
        userId: user.id,
  ...
  try {
    ...
    if (task.project_id) {
      ...
      if (
        ...
        !projectAccess.accessibleProjectIds.includes(task.project_id)
      ) {
        return NextResponse.json(
          { success: false, error: "Nemáte oprávnenie odstrániť úlohu z tohto projektu" },
          { status: 403 }
        );
      }
    }
