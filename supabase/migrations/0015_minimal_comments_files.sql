-- Minimal migration for task comments and files (without foreign keys for now)
...
-- Create task_comments table (without foreign key for now)
CREATE TABLE IF NOT EXISTS public.task_comments (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    task_id UUID NOT NULL, -- No foreign key for now
    user_id UUID NOT NULL,
    content TEXT NOT NULL,
    content_html TEXT, -- For rich text formatting
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    parent_id UUID, -- No foreign key for now
    is_edited BOOLEAN DEFAULT FALSE
);

-- Create task_files table (without foreign key for now)
