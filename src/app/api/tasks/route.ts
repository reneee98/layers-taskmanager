...
export async function GET(request: NextRequest) {
  try {
    ...
    if (projectId) {
      if (restrictedProjectIds && !restrictedProjectIds.includes(projectId)) {
        return NextResponse.json({ success: true, data: [] });
      }
    ...
    const [workspaceMembersResult, profilesResult, timeEntriesResult] = await Promise.all([
      ...
      assigneeUserIds.length > 0
        ...
        : Promise.resolve({ data: [] }),
      supabase.from("time_entries").select("task_id, amount").in("task_id", taskIds),
    ]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!projectAccess.hasFullProjectAccess) {
      const targetProjectId = validation.data.project_id || null;
      const canWriteToProject = targetProjectId
        ? projectAccess.accessibleProjectIds.includes(targetProjectId)
        : false;

      if (!canWriteToProject) {
        return NextResponse.json(
          { success: false, error: "Nemáte oprávnenie vytvoriť úlohu mimo pridelených projektov" },
          { status: 403 }
        );
      }
    }
