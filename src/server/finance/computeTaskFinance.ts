...
export const computeTaskFinance = async (
  ...
  // 8. Calculate totals
  // For tasks: totalCost should be laborCost (actual cost of work) + externalCost
  // This represents the actual cost spent on the task
  const totalCost = laborCost + externalCost;

  // 9. Calculate profit/loss
  // Profit = Budget - Total Cost (what we should invoice minus what we spent)
  // For tasks, revenue is calculated from time entries, but profit should be budget - costs
  const profit = budgetAmount > 0 ? (budgetAmount - totalCost) : (revenue - totalCost);
  const profitPct = budgetAmount > 0 ? (profit / budgetAmount) * 100 : (revenue > 0 ? (profit / revenue) * 100 : 0);
