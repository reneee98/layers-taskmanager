...
export default function InvoicesPage() {
  ...
  const totalProjectsValue = projects.reduce((sum, project) => sum + project.total_cost, 0);
  const totalTasksValue = tasks.reduce((sum, task) => sum + task.total_cost, 0);
          ...
          {tasks.length === 0 ? (
            <Card>
              <CardContent className="text-center py-8">
                ...
                <p className="text-muted-foreground">Žiadne dokončené úlohy</p>
              </CardContent>
            </Card>
          ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mb-2">
                          <strong>Projekt:</strong> {task.project.name} ({task.project.code}) • <strong>Klient:</strong> {task.project.client.name}
                        </p>
                      ...
                      <div className="text-right ml-4">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {task.labor_cost > 0 && `${formatCurrency(task.labor_cost)} (čas) `}
                          {task.fixed_budget_cost > 0 && `${formatCurrency(task.fixed_budget_cost)} (fixná) `}
                          {task.external_cost > 0 && `${formatCurrency(task.external_cost)} (náklady)`}
                        </div>
                      </div>
                    </div>
                  </CardContent>
