...
export async function GET(req: NextRequest) {
  try {
    ...
    const [workspaceResult, memberResult] = await Promise.all([
      ...
      supabase.from('workspace_members').select('id, role').eq('workspace_id', workspaceId).eq('user_id', user.id).single()
    ]);
    ...
    const [
      ...
      isRiesitel ? supabase.from("task_assignees").select("task_id").eq("user_id", user.id).eq("workspace_id", workspaceId) : supabase.from("task_assignees").select("task_id").eq("user_id", user.id).eq("workspace_id", workspaceId),
      // Get all tasks with assignees
      supabase.from("task_assignees").select("task_id").eq("workspace_id", workspaceId),
      // Workspace members
      supabase.from('workspace_members').select('id, role, user_id').eq('workspace_id', workspaceId),
      // Projects
      ...
      // Invitations
      supabase.from('workspace_invitations').select(`*, workspace:workspaces(id, name, description, owner_id)`).eq('email', user.email).eq('status', 'pending').gt('expires_at', new Date().toISOString()).order('created_at', { ascending: false })
    ]);
    ...
    const { data: profilesData } = userIdsArray.length > 0
      ? await supabase.from('profiles').select('id, email, display_name, role').in('id', userIdsArray)
      : { data: [] };
    ...
    const allTaskIdsWithAssignees = new Set(allTaskAssignees.data?.map((ta: any) => ta.task_id) || []);
    
    // OPTIMIZED: Fetch all active tasks in one query instead of 3 separate queries
    const { data: allActiveTasksData } = await supabase
      .from("tasks")
      .select(`id, title, description, status, priority, estimated_hours, actual_hours, start_date, end_date, due_date, created_at, updated_at, project_id, assignee_id, assigned_to, budget_cents, workspace_id, project:projects(id, name, code, workspace_id, client:clients(name))`)
      .eq("workspace_id", workspaceId)
      .neq("status", "done")
    ...
    const { data: allAssignees } = allTaskIds.size > 0
      ? await supabase.from("task_assignees").select("id, task_id, user_id, assigned_at, assigned_by").in("task_id", Array.from(allTaskIds)).eq("workspace_id", workspaceId)
      : { data: [] };
    ...
    const { data: assigneeProfiles } = validAssigneeUserIds.length > 0
      ? await supabase.from("profiles").select("id, display_name, email, role").in("id", validAssigneeUserIds)
      : { data: [] };
    ...
    const activityProjectIds = Array.from(new Set(activities.map((a: any) => a.project_id).filter(Boolean)));
    const activityTaskIds = Array.from(new Set(activities.map((a: any) => a.task_id).filter(Boolean)));
    const activityUserIds = Array.from(new Set(activities.map((a: any) => a.user_id).filter(Boolean)));

    const [activityProjects, activityTasks, activityUsers] = await Promise.all([
      activityProjectIds.length > 0 ? supabase.from("projects").select("id, name, code").in("id", activityProjectIds) : Promise.resolve({ data: [] }),
      activityTaskIds.length > 0 ? supabase.from("tasks").select("id, title").in("id", activityTaskIds) : Promise.resolve({ data: [] }),
      activityUserIds.length > 0 ? supabase.from("profiles").select("id, display_name, email, avatar_url").in("id", activityUserIds) : Promise.resolve({ data: [] })
    ]);
